...
define(['N/record', 'N/log', 'N/search', 'N/runtime'], function (record, log, search, runtime) {
    ...
    function getInputData() {
        ...
        const recType = scriptObj.getParameter({ name: 'custscript_ic_mr_rec_type' }) || 'salesorder'
        const singleRecId = scriptObj.getParameter({ name: 'custscript_ic_mr_rec_id' })
        const searchId = scriptObj.getParameter({ name: 'custscript_ic_mr_search_id' })

        if (singleRecId) {
            return [{ id: singleRecId, type: recType }]
        }
        ...
        if (searchId) {
            ...
            savedSearch.run().each(function (result) {
                ids.push({ id: result.id, type: result.recordType || recType })
                return true
            })
            return ids
        }
        ...
        log.error({
            ...
            details: 'Set either custscript_ic_mr_rec_id or custscript_ic_mr_search_id script parameter.'
        })
        return []
    }
    ...
    function map(context) {
        let input
        try {
            input = JSON.parse(context.value)
        } catch (e) {
            log.error({
                title: 'Failed to parse map input',
                details: e.message || String(e)
            })
            return
        }
        ...
        try {
            ...
            log.error({
                title: `Failed to load source record ${input.id}`,
                details: e.message || String(e)
            })
            return
        }
